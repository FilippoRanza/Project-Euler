...
def find_truncatable_primes(curr_primes: List[Long], missing: Int, acc: List[Long]): List[Long] = 
  if missing == 0 then acc
    ...
    if is_truncatable(curr_p, primes) then find_truncatable_primes(primes, missing - 1, acc :+ curr_p)
    else find_truncatable_primes(primes, missing, acc)

def is_truncatable(curr_p: Long, curr_primes: List[Long]): Boolean = 
  search_trunc(truncate_right(curr_p), curr_primes) && search_trunc(truncate_left(curr_p), curr_primes)


def search_trunc(trunc: LazyList[Long], curr_primes: List[Long]): Boolean = 
  trunc match
    case h #:: tail => if bin_search(curr_primes, h).isDefined then search_trunc(tail, curr_primes) 
                        else false
    case _ => true
